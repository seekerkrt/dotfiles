...
通常端末側では認証が正常な場合があるため、Codex sandbox内から認証状態を修復しない。
...
`gh`が利用できない、または`gh`で必要情報を取得できない場合は、利用可能なGitHub連携機能を使用する。

fallback時も次を守る。

- 対象repositoryを明示する
- Issue番号、PR番号、branch、commitを可能な限り特定する
- connector結果とローカルGit状態を混同しない
- 書き込み操作はユーザーの明示依頼時だけ行う
- 結果確認を可能な範囲で行う
- 取得できない情報は「未確認」とする
